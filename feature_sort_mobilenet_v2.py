...
np.save(f"imagnet_labels.npy", total_labels)
...
for j in range(1000):
    ...
    for index in indexes:
        ...
        np.save(f"class_{j}.npy".format(j=j), total_features_j[1:])

val1 = np.load(f"val_features_100.npz")
val2 = np.load(f"val_features_200.npz")
val3 = np.load(f"val_features_300.npz")
val4 = np.load(f"val_features_390.npz")
