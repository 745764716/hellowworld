...
class DenseNet3(nn.Module):
    def __init__(self, args, depth, num_classes, growth_rate=12,
        ...
        # self.fc = RouteDICE(in_planes, num_classes, p=90, info=np.load(f"./models/dice/{args.in_dataset}_{args.model_arch}_feat_stat.npy")) #dice

        # self.fc = RouteLUNCH(in_planes, num_classes, p_w=90, p_a=90, info=np.load(f"./models/line/{args.in_dataset}_{args.model_arch}_feat_stat.npy")) #line
    ...
    def feature_list(self, x):
        ...
        # out = torch.clamp(out, max=1.5)  #react

        # out = ash_s(out, percentile=90) #ash
    ...
    def intermediate_forward(self, x, layer_index=None):
        ...
        out = self.relu(self.bn1(out))
        out = F.avg_pool2d(out, 8)
        out = torch.clamp(out, max=0.25)
        # out = ash_s(out, percentile=70)  # ash
        # if layer_index == 1:
        #     out = self.trans1(self.block1(out))
        # elif layer_index == 2:
        #     out = self.trans1(self.block1(out))
