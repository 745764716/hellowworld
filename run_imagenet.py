...
feat_log_val = np.load(f"cache/imagenet_feature_resnet_50_sorted/val.npy")
ftest = normalizer(prepos_feat(feat_log_val))

score_log_exp = []
class_protytypes = np.zeros((class_num, 2048))

model = resnet50(num_classes=1000, pretrained=True).fc
model.cuda()
model.eval()

for k in range(class_num):
    k_ftrain = np.load(f"cache/imagenet_feature_resnet_50_sorted/class_{k}.npy".format(k=k))
